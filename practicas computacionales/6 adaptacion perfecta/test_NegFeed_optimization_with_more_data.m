...
[t_1,y_1] = ode45(@(t,y)negative_feedback_network(t,y, k1, k2, k3, k4,S),tspan,y0);
...
[t_2A,y_2A] = ode45(@(t,y)negative_feedback_network(t,y, k1, k2, k3, k4,S),tspan_2A,y0_2A);
%%
tspan_2B=[0 6];
y0_2B=y_2A(end,:); % R(0), X(0)
S=0.1;

[t_2B,y_2B] = ode45(@(t,y)negative_feedback_network(t,y, k1, k2, k3, k4,S),tspan_2B,y0_2B);
...
%%
title('Neg feedback','FontSize', 12);

legend('optimized neg feedback model (to data from experiment 1)',...
       'Experiment 1','Experiment 2','neg feedback model simulation of exp 2 (pre)',...
       'neg feedback model simulation of exp 2 (exp)');
